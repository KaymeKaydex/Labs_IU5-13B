...
double maxElement(vector<vector<double>> a, int nrow, int ncol) {
	double max = NULL;
	for (int i = 0; i < nrow; i++)
	{
		for (int j = 0; j < ncol; j++)
...
template <typename T>
void ShowMatrix(T** a, int nrow, int ncol, int nZnakov = 0, bool scient = 0)
{
	cout << string(40, '=') << endl;
...
void ShowMatrix(vector<vector<double>> a, int nrow, int ncol, int nZnakov = 0, bool scient = 0)
{
	cout << string(40, '=') << endl;
	int LenghtOfConsole = 80;
	int w = Razryad(maxElement(a, nrow, ncol)) + nZnakov + 5;
